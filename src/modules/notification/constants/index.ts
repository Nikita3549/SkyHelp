export const CREATE_CLAIM_FILENAME = 'createClaim.html';

export const FINISH_CLAIM_FILENAME = 'finishClaim.html';

export const GENERATE_NEW_ACCOUNT_FILENAME = 'generateNewAccount.html';

export const NEW_STATUS_FILENAME = 'newStatus.html';

export const DOCUMENT_REQUEST_FILENAME = 'documentRequests.html';

export const MISSING_DOCUMENTS_FILENAME = 'missingDocuments.html';

export const SEND_PARTNER_PAYOUT_FILENAME = 'sendPartnerPayout.html';
